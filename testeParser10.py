...
def inExp(t, tree, aux = None): #Em construção
    while True:
        no = Node('EXP')
        tree.addChild(no)
        ...
        if(t.type == 'IF'):#SE FOR IF
            ...
            if(t.type == 'THEN'):
                ...
                if(t.type == 'ELSE'):
                    ...
                    if(isExp(t)):
                        t = inExp(t, no)
                        t = inExp2(t, no)#auhUShUDHAUSHdçAUSHdçAUSHdçUAHSdçuAHSDçUASHDçuAHSÇDuhAS
                    else:
                        error(t, 'EXPRESSÃO')
                        break
                    if(t.type == 'FI'):
